# -*- coding: utf-8 -*-
"""
Script Structural Check IRIO Indonesia using Moore-Penrose Inverse
...
def error_check(vector_x=None, vector_fd=None, matrix_z=None, vektor_e=None,
    ...
    mat_output = vector_x * np.identity(len(vector_x))
    inv_x = np.linalg.pinv(mat_output) # Moore-Penrose inverse
    mat_A = matrix_z @ inv_x
...
def adj_x(vector_fd=None, matrix_z=None):
    ...
    mat_output = vector_x_new * np.identity(len(vector_x_new))
    inv_x = np.linalg.pinv(mat_output) # Moore-Penrose inverse
    mat_A = matrix_z @ inv_x
